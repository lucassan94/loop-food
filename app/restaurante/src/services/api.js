...
api.interceptors.request.use((config) => {
  // Cookie própria do app admin — sessões individuais por módulo
  const token = document.cookie.match(/(^| )admin_publicToken=([^;]+)/)?.[2]
...
api.interceptors.response.use(
  ...
  async (error) => {
    if (error.response?.status === 401 && !error.config._retry) {
      ...
      const hadToken = !!document.cookie.match(/(^| )admin_publicToken=([^;]+)/)?.[2]
      if (!hadToken) return Promise.reject(error)

      error.config._retry = true
      try {
        await axios.post('/api/auth/refresh', {}, { withCredentials: true, headers: { 'X-Module': 'admin' } })
