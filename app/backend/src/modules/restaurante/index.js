...
router.get('/equipe', authenticate, authorize('admin', 'gerente'), async (req, res, next) => {
  try {
    const restaurantId = req.restaurantId || config.restaurantId;
    const result = await query(
      'SELECT id, nome, email, apelido, cargo, ativo, ultimo_acesso, criado_em FROM restaurante_users WHERE restaurant_id = $1',
      [restaurantId]
    );
    res.json(result.rows);
  } catch (err) {
...
router.post('/equipe', authenticate, authorize('admin', 'gerente'), async (req, res, next) => {
  try {
    const restaurantId = req.restaurantId || config.restaurantId;
    const { nome, apelido, email, password, cargo } = req.body;
    const senhaHash = await bcrypt.hash(password, 12);

    // Apelido (username de login) em minúsculas, igual ao login
    const apelidoNormalizado = apelido ? String(apelido).trim().toLowerCase() : null;
    const emailFinal = email !== undefined ? email : '';

    if (apelidoNormalizado && apelidoNormalizado.length < 2) {
      throw new AppError('Apelido deve ter no mínimo 2 caracteres.', 400);
    }

    if (apelidoNormalizado) {
      const existente = await query(
        'SELECT id FROM restaurante_users WHERE restaurant_id = $1 AND apelido = $2',
        [restaurantId, apelidoNormalizado]
      );
    ...
    const result = await query(
      `INSERT INTO restaurante_users (restaurant_id, nome, email, apelido, senha_hash, cargo)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, nome, email, apelido, cargo`,
      [restaurantId, nome, emailFinal, apelidoNormalizado, senhaHash, cargo || 'caixa']
    );
...
router.put('/equipe/:id', authenticate, authorize('admin', 'gerente'), async (req, res, next) => {
  try {
    const restaurantId = req.restaurantId || config.restaurantId;
    const { id } = req.params;
    const { nome, apelido, email, password, cargo, ativo } = req.body;
    ...
    if (apelido !== undefined && apelido !== '') {
      const apelidoNormalizado = String(apelido).trim().toLowerCase();
      if (apelidoNormalizado.length < 2) {
        throw new AppError('Apelido deve ter no mínimo 2 caracteres.', 400);
      }
      const existente = await query(
        'SELECT id FROM restaurante_users WHERE restaurant_id = $1 AND apelido = $2 AND id != $3',
        [restaurantId, apelidoNormalizado, id]
      );
      if (existente.rows.length > 0) {
        throw new AppError('Este apelido já está em uso.', 409);
      }
      fields.push(`apelido = $${paramIdx++}`);
      params.push(apelidoNormalizado);
    }
    if (email !== undefined) {
    ...
    const result = await query(
      `UPDATE restaurante_users SET ${fields.join(', ')} WHERE id = $1 AND restaurant_id = $2
       RETURNING id, nome, email, apelido, cargo, ativo`,
      params
    );
...
router.put('/seguranca', authenticate, authorize('admin'), async (req, res, next) => {
  try {
    const { apelido, senha_atual, nova_senha } = req.body;
    const { id } = req.user;
    ...
    // Atualizar apelido (username de login) — ignora vazio, normaliza minúsculas e valida duplicidade
    if (apelido !== undefined && apelido !== '') {
      const restaurantId = req.restaurantId || config.restaurantId;
      const apelidoNormalizado = String(apelido).trim().toLowerCase();
      ...
      await query('UPDATE restaurante_users SET apelido = $1 WHERE id = $2', [apelidoNormalizado, id]);
    }
