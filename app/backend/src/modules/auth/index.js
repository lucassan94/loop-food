...
const loginClienteSchema = z.object({
  ...
  password: z.string().min(8, 'Senha deve ter no mínimo 8 caracteres.'),
}).refine((d) => d.telefone || d.apelido, {
  message: 'Informe usuário (apelido) ou telefone.',
});
...
router.post('/cliente/login', loginLimiter, async (req, res, next) => {
  try {
    ...
    const { telefone, apelido, password } = loginClienteSchema.parse(req.body);

    // Suporta login por USERNAME (apelido) OU por telefone:
    //  - apelido explícito no body → modo apelido
    //  - telefone com < 8 dígitos (ex: 'cliente') → trata como apelido
    //  - telefone com máscara/dígitos → modo telefone
    const identificador = (apelido || telefone || '').trim();
    const digitos = identificador.replace(/\D/g, '');
    const modoApelido = !!apelido || digitos.length < 8;

    let result;
    if (modoApelido) {
      result = await query(
        `SELECT id, nome, sobrenome, apelido, email, telefone, endereco, numero, bairro, complemento, cidade, estado, cep, cpf_cnpj, senha_hash
         FROM clientes
         WHERE restaurant_id = $1 AND LOWER(apelido) = LOWER($2) AND ativo = true`,
        [restaurantId, identificador]
      );
    } else {
      result = await query(
        `SELECT id, nome, sobrenome, apelido, email, telefone, endereco, numero, bairro, complemento, cidade, estado, cep, cpf_cnpj, senha_hash
        ...
        [restaurantId, `%${digitos}`]
      );
    }

    const user = result.rows[0];
    if (!user) throw new AppError('Usuário/telefone ou senha inválidos.', 401);
...
router.post('/entregador/login', loginLimiter, async (req, res, next) => {
  try {
    const restaurantId = req.restaurantId || config.restaurantId;
    const { telefone, apelido, password } = loginEntregadorSchema.parse(req.body);

    // Suporta login por USERNAME (apelido) OU por telefone (mesma lógica do cliente)
    const identificador = (apelido || telefone || '').trim();
    const digitos = identificador.replace(/\D/g, '');
    const modoApelido = !!apelido || digitos.length < 8;
    ...
    if (modoApelido) {
      result = await query(
        `SELECT id, nome, apelido, email, telefone, senha_hash, status, entregas_total, frete_total_recebido
         FROM entregadores
         WHERE restaurant_id = $1 AND LOWER(apelido) = LOWER($2)`,
        [restaurantId, identificador]
      );
    } else {
      result = await query(
...
router.post('/restaurante/login', loginLimiter, async (req, res, next) => {
  try {
    ...
    const senhaValida = await bcrypt.compare(password, user.senha_hash);
    if (!senhaValida) throw new AppError('Apelido ou senha inválidos.', 401);
...
router.get('/me', authenticate, async (req, res, next) => {
  try {
    ...
    if (role === 'cliente') {
      const result = await query(
        'SELECT id, nome, sobrenome, apelido, email, telefone, endereco, cep, numero, bairro, complemento, cidade, estado, cpf_cnpj FROM clientes WHERE id = $1',
        [id]
      );
    ...
    if (role === 'entregador') {
      const result = await query(
        'SELECT id, nome, apelido, email, telefone, status, foto_url, entregas_total, frete_total_recebido, endereco FROM entregadores WHERE id = $1',
        [id]
      );
      if (!result.rows[0]) throw new AppError('Usuário não encontrado.', 404);
