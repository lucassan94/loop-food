...
// Staff loga por apelido (username), cliente por telefone
function login(apelido, password) {
  return fetch(API + '/auth/restaurante/login', {
    method: 'POST',
    body: { apelido, password },
  });
}

function loginCliente(telefone, password) {
  return fetch(API + '/auth/cliente/login', {
    method: 'POST',
    body: { telefone, password },
  });
}
...
async function main() {
  ...
  await test('SETUP - Login Gerente (ou criar)', async () => {
    const res = await login('gerente', 'gerente123');
    if (res.status === 200) {
      ...
      const create = await fetch(API + '/restaurante/equipe', {
        ...
        body: { nome: 'Gerente Teste', apelido: 'gerente', password: 'gerente123', cargo: 'gerente' },
      });
      assertStatus(create, 201, 'Criar gerente');
      const loginRes = await login('gerente', 'gerente123');
  ...
  await test('SETUP - Login Chef (ou criar)', async () => {
    const res = await login('chef', 'chef123');
    if (res.status === 200) {
      ...
      const create = await fetch(API + '/restaurante/equipe', {
        method: 'POST',
        headers: { Authorization: 'Bearer ' + adminToken },
        body: { nome: 'Chef Teste', apelido: 'chef', password: 'chef123', cargo: 'chef' },
      });
      assertStatus(create, 201, 'Criar chef');
      const loginRes = await login('chef', 'chef123');
  ...
  await test('SETUP - Login Caixa (ou criar)', async () => {
    const res = await login('caixa', 'caixa123');
    if (res.status === 200) {
      ...
      const create = await fetch(API + '/restaurante/equipe', {
        method: 'POST',
        headers: { Authorization: 'Bearer ' + adminToken },
        body: { nome: 'Caixa Teste', apelido: 'caixa', password: 'caixa123', cargo: 'caixa' },
      });
      assertStatus(create, 201, 'Criar caixa');
      const loginRes = await login('caixa', 'caixa123');
  ...
  await test('SETUP - Criar entregador de teste', async () => {
    ...
    const create = await fetch(API + '/entregadores', {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + adminToken },
      body: { nome: 'Entregador RLS', email: 'entregador.rls@teste.com', telefone, password: 'entregador123' },
    });
    ...
    const loginRes = await fetch(API + '/auth/entregador/login', {
      method: 'POST',
      body: { telefone, password: 'entregador123' },
    });
  ...
  await test('SETUP - Criar pedido de teste', async () => {
    ...
    // Login cliente
    const cliRes = await loginCliente('(11) 99999-8888', 'cliente123');
  ...
  await test('Cliente: login', async () => {
    const res = await loginCliente('(11) 99999-8888', 'cliente123');
