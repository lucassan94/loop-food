"""Suíte de testes da API (módulo Backend) contra o tenant LOOP.

Base: o script original (trash/tmp_api_tests.py) rodava contra a instância
deployada (tenant 1). Esta versão roda contra o tenant LOOP enviando o header
`X-Tenant-Slug: loop` em TODAS as requisições — funciona tanto no backend
local (localhost:3001) quanto no deployado (86.48.18.22:8090), sem depender
do RESTAURANT_ID do processo.

Diferenças vs. original:
  * BASE + header X-Tenant-Slug configuráveis por env (API_BASE / TENANT_SLUG).
  * Cliente de teste criado SEMPRE via signup (telefone aleatório) — não usa o
    seed 'cliente' para não alterar o telefone dele no PUT /clientes/perfil.
  * Loga o tenant resolvido via /health para confirmar que está no Loop.
"""
...
BASE = os.environ.get("API_BASE", "http://localhost:3001/api")
SLUG = os.environ.get("TENANT_SLUG", "loop")
results = []
...
opener_cliente = novo_opener() # signup/login cliente + rotas de cliente
...
def req(opener, method, path, body=None, token=None, origin=None):
    ...
    headers = {
        ...
        "X-Tenant-Slug": SLUG,  # resolve o tenant LOOP em todas as chamadas
    }
    if token:
...
# ══════════ Setup: confirmar tenant ══════════
s, p, _ = req(opener_anon, "GET", "/health")
print(f"[setup] health status={s} restaurantId={p.get('restaurantId') if isinstance(p, dict) else None} "
      f"tenant={p.get('tenant') if isinstance(p, dict) else None}")

# ══════════ API-01 health ══════════
s, p, _ = req(opener_anon, "GET", "/health")
check("API-01", "health 200", s == 200, f"(status={s}, tenant={p.get('tenant') if isinstance(p, dict) else None})")
...
# ══════════ Signup cliente de teste (telefone aleatório — não suja o seed) ══════════
telefone_teste = "(11) 9" + str(random.randint(10000000, 99999999))
email_teste = f"api-teste-{random.randint(100000, 999999)}@loop.test"  # email único evita 409 (UNIQUE restaurant_id+email)
s, p, _ = req(opener_cliente, "POST", "/auth/cliente/signup", {
    "nome": "Cliente Teste", "sobrenome": "API Loop", "telefone": telefone_teste,
    "email": email_teste, "password": "cliente123",
})
cliente_token = p.get("token") if isinstance(p, dict) else None
print(f"[setup] signup cliente de teste status={s} (telefone {telefone_teste})")
if not cliente_token:
    # Fallback: usuário seed 'cliente' (apenas leitura — API-15/17 podem falhar se mudar o telefone)
    ...
    print(f"[setup] fallback login apelido 'cliente' status={s1}")
...
telefone_unico = "(11) 9" + str(random.randint(10000000, 99999999))
...
fresh = novo_opener()
try:
    resp = fresh.open(urllib.request.Request(BASE + "/pedidos", method="GET", headers={"X-Tenant-Slug": SLUG}), timeout=30)
    s = resp.status
except urllib.error.HTTPError as e:
    s = e.code
except Exception:
...
# ══════════ API-20 CORS ══════════
s, p, h = req(opener_anon, "GET", "/produtos", origin="http://localhost:5174")
