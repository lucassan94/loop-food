  ...
  try {
    ...
    const adminResult = await client.query(
      `INSERT INTO restaurante_users (restaurant_id, nome, email, senha_hash, cargo)
       VALUES ($1, 'Admin SaborExpress', 'admin@saborexpress2.com', $2, 'admin')
    ...
    // ─── 8. CLIENTE DE TESTE para Tenant 2 ───
    ...
    const clienteResult = await client.query(
      `INSERT INTO clientes (restaurant_id, nome, sobrenome, email, telefone, senha_hash, endereco, cep, bairro, cidade, estado)
       VALUES ($1, 'João', 'Santos', 'joao@email.com', '(11) 97777-6666', $2, 'Rua Augusta, 500', '01305-000', 'Consolação', 'São Paulo', 'SP')
       ON CONFLICT (restaurant_id, email) DO NOTHING
       RETURNING id`,
      [tenant2Id, clienteHash]
    );
    if (clienteResult.rows.length > 0) {
      console.log('   ✅ Cliente criado: joao@email.com / cliente123');
    } else {
