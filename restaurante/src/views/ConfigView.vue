    ...
    <div v-if="activeTab === 'geral'" style="display:grid;gap:1.5rem;max-width:800px;">
      ...
      <div class="card">
        ...
        <div class="card-body" style="border-top:1px solid var(--border);padding-top:1rem;display:flex;align-items:center;justify-content:space-between;">
          <div>
            <strong><i-lucide-truck v-if="!modoSemEntregador" style="width:16px;height:16px" /><i-lucide-x-circle v-else style="width:16px;height:16px" /> Modo {{ modoSemEntregador ? 'Sem Entregador' : 'com Entregador' }}</strong>
            <p style="font-size:0.85rem;color:var(--text-muted);margin-top:4px;">
              {{ modoSemEntregador ? 'Restaurante gerencia entregas manualmente.' : 'Entregadores gerenciam status de entrega.' }}
            </p>
          </div>
          <label class="toggle">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <table class="data-table" v-if="equipe.length">
            <thead><tr><th>Nome</th><th>E-mail</th><th>Cargo</th><th>Status</th></tr></thead>
            <tbody>
    ...
    <div v-if="editUserModal" class="modal-backdrop" @click.self="editUserModal = false">
      <div class="modal-card">
        ...
        <div class="modal-body">
          ...
          <div class="form-group"><label>E-mail</label><input v-model="editUserForm.email" type="email" placeholder="email@exemplo.com" /></div>
          <div class="form-group">
...
function editarUsuario(u) {
  editUserForm.id = u.id; editUserForm.nome = u.nome; editUserForm.email = u.email
...
async function salvarEdicaoUsuario() {
  ...
  try {
    const payload = { nome: editUserForm.nome, email: editUserForm.email, cargo: editUserForm.cargo, ativo: editUserForm.ativo }
